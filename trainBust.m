...
##  @var{prtdir} - Optional argument. Output directory for plots if toprt is true.
##
##Outputs:@*
...
## @end deftypefn
function [H, INFO] = trainBust(H, Shifts, toplt = false, toprt = false, ...
                               middle = 'm', prtdir = '../impStepPlts')
  A = H;
  ...
  do
    ...
    if(toplt)
      pltMat(H);
      if(toprt)
	    fn = sprintf('%s/impstep%03d.eps', prtdir, ++pltNum);
        print(fn, '-deps', '-tight');
      else
        pause(_PAUSELEN);
      endif
